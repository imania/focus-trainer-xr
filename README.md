# Focus Trainer XR

Meta Quest 2에서 사용할 수 있는 눈 초점 전환/추적 운동용 WebXR MVP입니다.

## 기능

- 왼쪽 눈, 오른쪽 눈, 양쪽 눈, 좌우 교대 운동 선택
- 왼쪽/오른쪽 눈별 균형, 근시 성향, 원시 성향 프로파일 선택
- 근시 성향은 원거리 체류를, 원시 성향은 근거리 체류를 더 강조
- 초점 거리 범위를 0.4m-8.0m 안에서 조절
- 가까이-멀리 초점 전환, 단계 랜덤 초점, 좌우 추적, 상하 추적, 원형 추적 패턴
- 기본 3초 간격으로 거리를 단계적으로 바꾸고 위치를 랜덤화하는 운동
- 바닥 격자, 깊이 프레임, 거리 축을 사용한 원근감 배경
- 1-10분 세션 타이머
- 반복 횟수와 현재 운동 눈 표시
- 일반 브라우저 3D 미리보기 및 Quest Browser WebXR 진입
- Meta Horizon Store용 immersive WebXR PWA 기본 구성

## 실행

정적 파일 서버가 있으면 이 폴더를 서비스하면 됩니다.

```powershell
python serve.py
```

또는 기본 정적 서버로도 미리보기가 가능합니다.

```powershell
python -m http.server 8080
```

Node 런타임을 선호하면 다음 스크립트도 사용할 수 있습니다.

```powershell
node server.js
```

PC 브라우저에서는 `http://localhost:8080`으로 미리보기를 확인할 수 있습니다.

Quest 2의 몰입형 WebXR은 보통 HTTPS 보안 컨텍스트가 필요합니다. 실제 헤드셋 테스트는 다음 중 하나를 권장합니다.

- GitHub Pages, Cloudflare Pages, Netlify 같은 HTTPS 정적 호스팅에 배포
- Cloudflare Tunnel 또는 ngrok으로 로컬 서버를 HTTPS 주소로 노출
- 이후 Unity/OpenXR APK로 포팅하여 Quest에 사이드로드

Quest Browser에서 HTTPS 주소로 접속한 뒤 `Enter VR`을 누르면 됩니다.

## PWA 패키징 준비

이 앱은 `manifest.webmanifest`, `service-worker.js`, 앱 아이콘을 포함합니다.
Meta Horizon Store용 PWA 패키징 절차는 `docs/meta-horizon-pwa-packaging.md`를 참고하세요.

```powershell
python scripts\validate-pwa.py
```

## 개발 메모

Quest 2는 눈 추적 센서가 없으므로 사용자가 실제로 어느 눈을 움직였는지 측정하지 않습니다. 대신 왼쪽/오른쪽 렌더 패스를 나누어 선택한 눈에는 선명한 타겟을, 반대쪽 눈에는 낮은 대비의 가이드만 보여줍니다.

근시/원시 프로파일은 치료 처방이 아니라 운동 자극의 체류 위치를 바꾸는 설정입니다.
한쪽 눈 또는 교대 모드에서는 해당 눈의 프로파일을 적용합니다. 양안 모드에서 양쪽 눈 프로파일이 서로 다르면 좌우 불일치 자극을 피하기 위해 균형 프로파일로 표시합니다.

Quest 2 같은 일반 VR 헤드셋은 광학 초점면이 고정되어 있으므로 가상 거리 변화가 실제 수정체 초점 거리 변화를 그대로 만들지는 않습니다. 이 앱의 거리 변화는 주로 깊이감, 수렴/분산, 추적 루틴을 위한 시각 자극입니다.

## 안전 고지

이 앱은 의료 진단, 치료, 시력 교정 효과를 보장하지 않습니다. 통증, 두통, 복시, 어지러움,
구역감이 발생하면 즉시 중단하고 필요하면 안과 전문의와 상담하세요.
